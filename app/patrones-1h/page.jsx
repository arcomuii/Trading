...
const SETTINGS_SCOPE = 'patrones-1h';
...
export const BITUNIX_TICKERS = [
  'CROSS', 'NIGHT', 'STBL', 'CTK', 'TREE', 'TURBO', 'H', 'CFG', 'CRWV', 'FIGHT', 'MEGA', 'NBIS', 'OPEN', 'SLX', 'ZAMA', 'APT', '0G', 'ZIL', 'G', 'RVN', 'CVX', 'ANIME', 'FLUID', 'ROSE', 'ZEREBRO', 'CETUS', 'RARE', 'RAY', 'VVV', 'THETA', 'MASK', 'STRK', 'BLUR', 'HYPER', 'MORPHO', 'AIO', 'COAI', 'DOGS', 'GMX', 'LYN', 'MAGMA', 'MAVIA', 'STEEM', 'TA', 'TOWNS', 'ZKC', 'EGLD', 'ENS', 'ILV', 'DRIFT', 'AERO', 'AXS', 'ELSA', 'RENDER', 'SPK', 'ARB', 'DOT', 'NOT', 'ACT', 'BERA', 'JST', 'AIXBT', 'ARIA', 'ERA', 'HAEDAL', 'HEI', 'SIREN', '4', 'APEX', 'BABY', 'BAS', 'CHILLGUY', 'CHIP', 'COLLECT', 'CVC', 'FOLKS', 'HYPE', 'KITE', 'MANTRA', 'RECALL', 'ROBO', 'SXT', 'SYN', 'DOGE', 'FLOW', 'POWR', 'QTUM', 'ALICE', 'SUSHI', 'DYM', 'ESPORTS', 'ETC', 'MOVE', 'SAHARA', 'SAND', 'SSV', 'TLM', 'BIGTIME', 'CRV', 'KOMA', 'ME', 'MEME', 'METIS', 'NEAR', 'BIO', 'BROCCOLI', 'ESP', 'GWEI', 'KERNEL', 'LAB', 'MYX', 'NAORIS', 'OG', 'POL', 'PROMPT', 'PUNDIX', 'REZ', 'SFP', 'VANA', 'XTZ', 'ZRX', 'ARPA', 'DOLO', 'SEI', 'ONG', 'ZEN', 'ACU', 'ASR', 'CC', 'CELR', 'COTI', 'DIA', 'FORM', 'LA', 'PARTI', 'RLC', 'SIGN', 'SYRUP', 'WAL', 'WIF', 'XNY', 'BANANA', 'TAO', 'ETHFI', 'FET', 'LSK', 'RESOLV', 'RSR', 'SKYAI', 'STG'
];
...
export default function PatronesPage() {
    ...
    const runScan = async (coinsList) => {
        ...
        for (let i = 0; i < coinsList.length; i++) {
            ...
            try {
                const data = await fetchPatterns(`${coin.symbol.toUpperCase()}USDT`);
                if (!stale()) {
                    ...
                    if (data) {
                        ...
                        if (condsMet === conds.length && !levels?.extended) {
                            ...
                            if (isBacktestApexTarget(data) && levels && isFavorableTp2(levels)) {
                                logBacktestEntry({ coin, levels, isBull: bias === 'bullish', patternLabel: meta.label, origen: 'patrones-1h', capital: getTradeAmount(SETTINGS_SCOPE), leverage: getAutoTradeLeverage(SETTINGS_SCOPE) });
                            }
                        }
                    }
                }
                    ...
                    <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-800 p-5 mb-6">
                        ...
                        {allPatterns.length > 0 && (
                            <p className="text-xs text-gray-400 dark:text-slate-500">
                                ...
                                <span className="text-teal-500 dark:text-teal-400">{countByTab.cup} tazas</span> · {' '}
